...
class TestSubmissionWorkflow(GptConversationFrameworkBase):
    ...
    def test_submit_without_message_uses_instance_model(self):
        ...
        with patch(
            "mightydatainc_gpt_conversation.gpt_conversation.gpt_submit"
        ) as mock_submit:
    ...
    def test_submit_passes_shotgun_argument_to_gpt_submit(self):
        client = self.make_client()
        conversation = self.make_conversation(client=client)

        with patch(
            "mightydatainc_gpt_conversation.gpt_conversation.gpt_submit"
        ) as mock_submit:
            mock_submit.return_value = "reply"
            conversation.submit(shotgun=5)

        self.assertEqual(mock_submit.call_args.kwargs["shotgun"], 5)
